...
- 教科 / 単元 / 学習目標 / 評価観点（「知識・技能」「思考・判断・表現」「主体的に学習に取り組む態度」の3視点
  × 「基礎」「標準」「応用」の3段階、視点×段階ごとに各3〜4個） / 模範回答 / 誤答例
...
- 知識・思考・態度の3視点 × 基礎・標準・応用の3段階＝9スコア（各0-100点、評価基準の3視点×3段階に対応）
- コメント（生徒の回答のどの部分を根拠にその評価に至ったかを具体的に説明する。80文字制限は撤廃）
- 改善提案（今後に向けた具体的な改善提案）

### CombinedEvaluationResult（結果確認画面・出力で使用、3方式を同時に含む）

- 三観点評価（ABC）：知識・思考・態度それぞれのランク（「標準」段階の到達度が80点以上=A、50点以上=B、それ未満=C）
- 5段階評価（1〜5）
- 100点法（0〜100）
- コメント
- 改善提案
...
- 評価基準確認：AI提案を「知識・技能」「思考・判断・表現」「主体的に学習に取り組む態度」の3視点×基礎・標準・応用の3段階（視点×段階ごとに各3〜4個）に分けて提示 → チェックボックス選択＋自由記述（視点×段階ごとに追加可） → `/updateCriteria`でKV反映後に採点
- 採点結果：三観点評価（ABC）・5段階評価・100点法の3方式を同時に表示・出力する（三観点評価のランクは知識・思考・態度それぞれの「標準」段階到達度から算出）
